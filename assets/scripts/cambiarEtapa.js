...
$.ajax({
    ...
    success: function (data) {
        ...
        data.forEach(function (lote) {
            ...
            $nuevoItem.find('[data-title]').text(lote.lote);
            $nuevoItem.find('[data-subtitle]').text(lote.especie);
            ...
            $nuevoItem.click(function () {
                ...
                $.ajax({
                    url: '/api/inventario.php?lote=' + $(this).attr('data-value'),
                    type: 'GET',
                    dataType: 'json',
                    beforeSend: function () {
                    ...
                    success: function (data) {
                        data.forEach(function (stock) {
                            ...
                            $nuevoItem.find('[data-etapa]').text(stock.etapa);
                            $nuevoItem.find('[data-ubicacion]').text(stock.ubicacion);
                            $nuevoItem.find('[data-cantidad]').text(stock.cantidad + ' ' + stock.unidad_medida);

                            $nuevoItem.click(function () {
                                selectedStock = stock;
                                etapa_origen_id.val(stock.etapa_id);
                                ubi_origen_id.val(stock.ubicacion_id);
                                ubi_destino_id.val(stock.ubicacion_id);
                                $('[data-stock-origen]').text(stock.cantidad);
                                $('[data-max-salida]').text(stock.cantidad);
